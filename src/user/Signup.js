...
function Signup() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      // 서버로 회원가입 데이터 전송
      const response = await axios.post('http://localhost:3003/api/register', formData); // 서버 URL을 올바르게 설정하세요.
          ...
          {selectedUserType === 'Farmer' && (
            <>
              <Form.Item
                ...
                name="FarmLocation"
              >
                <Input name="FarmLocation" onChange={handleFormChange} />
              </Form.Item>
              <Form.Item
                label="재배 작물"
